...
# Այստեղ պետք է լիստ ունենանք, որը պարունակում է վերևի երեք լիստերը
print(lst)
print(lst1)
print(lst2)
...
lst = list(nonsense)
for words in lst:
  if 'b' in words:  # Պետք է առաջին տառը ստուգեինք չէ՞, ոչ թե առհասարակ b կա, թե չէ։ Ու կարելի ա upper() կամ lower()-ով
    #   ստուգել մեծատառ ու փոքրատառը
    Count1 += 1
  if 'B' in words:
    Count1 += 1
print(Count1)
...
print(result)
# Վատ չի, կոդի ֆորմատավորումն էլ ա լավ, բայց մի երկու տեղ մեկ ա թերի է։ Ֆունկցիաների մեջ էլ արգումենտները բացատով են
# առանձնացվում
