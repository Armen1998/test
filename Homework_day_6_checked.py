...
def alone(lst):  # Մի քիչ երկար ա ստացվել :)
...
def squares(lst):  # Իմաստ չունի արգումենտ տալ ֆունկցիային, եթե անմիջապես հետո վերգրելու ենք այն
   lst = [i ** 2 for i in range(30)]  # Տիրույթը պետք է լիներ 1-30 ներառյալ և առանց 0
   print(lst)
...
def rand(n):  # Պահանջվում է վերադարձնել այդ լիստը, ոչ թե տպել
    lst = [random.randint(-100, 400) for i in range(n) ]
    print(lst)
rand(5)
...
# def vowel(lst):
#   lst2 = [i for i in lst if i.upper().startswith(('A', 'O', 'I', 'U', 'E'))]
#   print(lst2)
#
# vowel(poem.split())
...
def six(a,b,c):  # Այստեղ էլ պետք է վերադարձվի, ոչ թե տպվի
    if 0 < abs(b) - a <= 1:
        if c - a >= 2:
            print(True)
    elif 0 < abs(c) - a <= 1:
...
assert six(10, 10, 8) == True, 'Function is wrong'  # Հետևյալ թեստը չի անցնում
...
#     return pi * r ** 2
#
# lam2 = lambda r, pi=3.14: pi * r ** 2
#
# def sum_to_power(x, y, p):
#     return (x + y) ** p
#
# lam3 = lambda x, y, p: (x + y) ** p
...
# print(lst1)
#
# newlst1 = filter(lambda x : x % 10 == 7,lst1)
#
# print(list(newlst1))
...
def ten(string):
    newword = ''
    for i in string:
        newword = newword + i + i  # ճիշտ է։ Կամ ավելի կարճ կարող ենք գրել newword += 2 * i, քանի որ բազմապատկման
        # օպերատորը սթրինգերի հետ էլ է աշխատում
    print(newword)
